    ...
    public function editProfile(Request $request){
        try{
            ...
            if($user->role->slug == 'seller'){
                ...
                $imageUploadPath = env('SELLER_PROFILE_IMAGE_UPLOAD').DIRECTORY_SEPARATOR.$sha1SellerId.DIRECTORY_SEPARATOR;

            }else{
                $sha1CustomerId = sha1(Customer::where('user_id', $user['id'])->pluck('id')->first());
                $imageUploadPath = env('CUSTOMER_PROFILE_IMAGE_UPLOAD').DIRECTORY_SEPARATOR.$sha1CustomerId.DIRECTORY_SEPARATOR;
            }
            $user->update([
